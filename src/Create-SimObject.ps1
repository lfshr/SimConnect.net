...
namespace SimConnectNet.Models
{
    ...
    {
        [BindSimVariable("{{Name}}")]
        public async Task<object> Get{{PascalName}}Async(){
        
        }

        [BindSimVariable("{{Name}}")]
        public async Task Set{{PascalName}}Async(object value){
...
$pascalMapping = $variableNames | ForEach-Object {
    @{Name = $_; PascalName = [string]::Join('', ($_ -split ' ' | %{(Get-Culture).TextInfo.ToTitleCase($_.ToLower())}))}
}

$pascalMapping | ForEach-Object {
    $content = $template -replace '{{Name}}', $_.Name -replace '{{PascalName}}', $_.PascalName

    Set-Content -Path ".\SimConnect.net\Models\SimObject$($_.PascalName).cs" -Value $content
}